...
- ユーザーには日本語で応答せよ。
- Markdownで意図的に改行を入れたい場合は、半角スペース2つやバックスラッシュの代わりに`<br>`タグを使用せよ。
- 破壊的変更を行う場合は、必ず事前に警告せよ。
- GitHub CLIの`gh`コマンドを使用可能。リモートリポジトリに変更を加える場合は、必ずユーザーに確認を求めよ。
- IssueやPull Requestの本文、または、長文の技術解説などは、`.gemini/tmp`フォルダに一時ファイル（例: `pr_body.md`）として出力し、そのファイル名と概要をユーザーに提示せよ。
...
1. **前提条件の確認**
    1. **メインブランチの同期:** ローカルのmasterブランチがリモートリポジトリの最新状態と一致していることを確認する。
